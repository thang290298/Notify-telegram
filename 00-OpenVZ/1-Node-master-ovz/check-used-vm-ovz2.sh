...
rm -rf /usr/local/src/diskuse.txt
rm -f /usr/local/src/USED.csv
...
/usr/sbin/vzctl exec $line df -h | grep /dev/pl | awk '{printf $2}' > /usr/local/src/disk.txt
cat /usr/local/src/disk.txt > /dev/null 2>&1
if [ $? = 0 ] and [ $? = 1 ]
then
disk=`/usr/sbin/vzctl exec $line df -h | grep /dev/pl | awk '{printf $2}'`
udisk=`/usr/sbin/vzctl exec $line df -h | grep /dev/pl | awk '{printf $5}'`
...
disk=`/usr/sbin/vzctl exec $line df -h | grep /dev/simfs | awk '{printf $2}'`
udisk=`/usr/sbin/vzctl exec $line df -h | grep /dev/simfs | awk '{printf $5}'`
sdisk=`echo $udisk | rev | cut -c 2- | rev`
